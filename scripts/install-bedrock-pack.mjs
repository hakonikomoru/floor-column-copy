#!/usr/bin/env node
/**
 * リポジトリの behavior / resource pack を Minecraft Launcher（Bedrock）の各配置先へジャンクションで接続する。
 */
...
const behaviorSrc = join(repoRoot, "behavior_packs", "floor_column_copy");
const resourceSrc = join(repoRoot, "resource_packs", "floor_column_copy");
...
if (!existsSync(join(resourceSrc, "manifest.json"))) {
  console.error(`Resource pack not found: ${resourceSrc}`);
  process.exit(1);
}
...
if (linked === 0) {
  ...
  console.log("");
  console.log("Done. Apply both packs in world settings, then try:");
  console.log("  !fc give");
}
